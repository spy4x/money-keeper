...
export class LoadGroupDataActionEffect {
  ...
  @Effect() watchCategories$ = this.actions$
    ...
    .pipe(
      ...
      filter(v => !!v),
      map(itemsArrays => itemsArrays.reduce((acc, cur) => ({...acc, ...cur}), {})),
      map((items: { [key: string]: Category[] }) => new CategoriesSetCollectionAction(items))
    );
  ...
  @Effect() watchExpenses$ = this.actions$
    ...
    .pipe(
      ...
      filter(v => !!v),
      map(itemsArrays => itemsArrays.reduce((acc, cur) => ({...acc, ...cur}), {})),
      map((items: { [key: string]: Expense[] }) => new ExpensesSetCollectionAction(items))
    );

  @Effect() watchTags$ = this.actions$
    .ofType(type)
    .pipe(
      withLatestFrom(this.store),
      switchMap(([action, state]: [LoadGroupDataAction, AppState]) => {
      ...
      filter(v => !!v),
      map(itemsArrays => itemsArrays.reduce((acc, cur) => ({...acc, ...cur}), {})),
      map((items: { [key: string]: Tag[] }) => new TagsSetCollectionAction(items))
    );

  @Effect() watchUsers$ = this.actions$
    .ofType(type)
    .pipe(
      withLatestFrom(this.store),
      switchMap(([action, state]: [LoadGroupDataAction, AppState]) => {
