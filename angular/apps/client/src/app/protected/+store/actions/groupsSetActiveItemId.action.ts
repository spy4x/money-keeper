...
export class GroupsSetActiveItemIdActionEffect {
  ...
  updateActiveGroupInUserConfigDoc$ = this.actions$.ofType(type).pipe(
    ...
    tap(({ groupId, userConfig }) =>
      this.db
        .doc(`usersConfigs/${userConfig.id}`)
        .update({ activeGroupId: groupId })
        .catch(error =>
          console.error('updateActiveGroupInUserConfigDoc$', error),
        ),
    ),
  );

  @Effect({ dispatch: false })
  loadGroupData$ = this.actions$.ofType(type).pipe(
    withLatestFrom(this.store),
    map(([action, state]: [GroupsSetActiveItemIdAction, AppState]) => ({
    ...
    tap(({ group }) =>
      this.snackBar.open(
        `Group is switched to "${
          group.isPersonal ? 'Personal expenses' : group.name
        }"`,
        undefined,
        { duration: 2500 },
      ),
