...
export class EditorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.initForm();
    this.sub.add(
      this.activatedRoute.params.pipe(filter(v => !!v)).subscribe(params => {
    ...
    this.sub.add(
      this.user$.pipe(filter(v => !!v), first()).subscribe(user => {
  ...
  editExpense(expenseId: string) {
    ...
    this.sub.add(
      this.store
        .pipe(
          select(getExpensesItemsForActiveGroup),
          filter(v => !!Object.keys(v).length),
          map(expenses => expenses[expenseId]),
          first(),
        )
  ...
  submit(): void {
    ...
    this.store.dispatch(
      expense.id
        ? new ExpensesEditAction(expense)
        : new ExpensesCreateAction(expense),
    );
  }
  ...
  getExpenseFromFormValue(): Expense {
    return {
      ...
      tags: this.form.value.tags.reduce(
  ...
  initForm(expense?: Expense) {
    this.form = this.fb.group({
      ...
      currency: [
        expense ? expense.currency.id : undefined,
        Validators.required,
      ],
      category: [
        expense ? expense.category.id : undefined,
        Validators.required,
      ],
      comment: [expense ? expense.comment : undefined],
      createdAt: [
        expense ? expense.createdAt : new Date(),
        Validators.required,
      ],
      tags: [expense ? Object.keys(expense.tags) : []],
      createdBy: [
        expense ? expense.createdBy.id : undefined,
        Validators.required,
      ],
    });
  }
  ...
  initGroupForm(group: Group) {
    ...
    this.sub.add(
      this.groupForm.valueChanges
        .pipe(filter(v => v.group !== this.groupId))
        .subscribe(groupFormValue => {
          this.groupId = groupFormValue.group;
          this.form.patchValue({
            // this prevents data from one group appear in another
            category: null,
