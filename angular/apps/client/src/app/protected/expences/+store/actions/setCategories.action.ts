...
export class CategoriesSetCollectionAction implements BaseAction<ProtectedState> {
    ...
    const categories = setStateProperties(
      ...
      groupIds.reduce((groupAcc, groupId) => ({
        ...
        [groupId]: {
          ...
          items: action.payload[groupId].reduce((categoryItems, category) => ({...categoryItems, [category.id]: category}),{})
        }
      }), {})
    );
