...
export class ListComponent {
  ...
  groups$ = this.store.pipe(
    select(getGroups),
    map(groups => groups.sort((a, b) => a.isPersonal === b.isPersonal ? 0 : a.isPersonal ? -1 : 1))
  );
  ...
  setActiveGroup(group: Group): void {
    this.store.dispatch(new GroupsSetActiveItemIdAction({groupId: group.id, shouldNotify: true}));
    this.router.navigate(['/']);
  }

  remove(group: Group): void {
    this.snackBar.open(`Delete group is not implemented yet.`, undefined, {duration: 2500});
  }
