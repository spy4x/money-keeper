...
export class CategoriesCreateActionEffect {
  ...
  save$ = this.actions$.ofType(type).pipe(
    ...
    tap(([action, state]: [CategoriesCreateAction, AppState]) => {
      ...
      this.db
        .collection<Category>(`groups/${activeGroup.id}/categories`)
        .add(category)
        .catch(error => {
          this.snackBar.open(
            `Category "${category.name}" wasn't saved to the server. Error`,
            undefined,
            { duration: 2500 },
          );
      ...
      this.snackBar.open(`Category "${category.name}" created`, undefined, {
        duration: 2500,
      });
      this.location.back();
    }),
