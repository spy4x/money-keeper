...
export class AuthSetUserActionEffect {
  ...
  authSetState$ = this.actions$
    ...
    .pipe(
      ...
      map(
        (action: AuthSetUserAction) =>
          new AuthSetStateAction(
            action.payload ? AuthState.loadingData : AuthState.notAuthenticated,
          ),
      ),
    );

  @Effect()
  authLoadData$ = this.actions$
    .ofType(type)
    .pipe(
      filter((action: AuthSetUserAction) => !!action.payload),
      map(
        (action: AuthSetUserAction) => new AuthLoadDataAction(action.payload),
      ),
    );
